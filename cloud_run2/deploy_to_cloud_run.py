...
# -*- coding: utf-8 -*-
"""
Google Cloud Run 2 (ADC / Agent Platform) 자동 빌드 및 배포 스크립트
1. GCP 인증 및 프로젝트(iceu-songpa09) 확인
2. 필수 GCP 서비스(Cloud Run, Cloud Build, Agent Platform/Vertex AI) 활성화
3. 서비스 계정에 Agent Platform 사용자 역할(roles/aiplatform.user) 부여
4. gcloud run deploy 실행 (Secret Manager 없이 순수 ADC 방식으로 컨테이너 배포)
5. 공인 HTTPS URL 확인 및 /api/health 및 Gemini 2.5 Flash 실시간 대화 테스트
6. 전체 로그 기록 (cloud_run2_deployment.log)
"""
...
class DeploymentLogger:
    def __init__(self, log_path):
        ...
        self.banner("Google Cloud Run 2 (ADC / Agent Platform) 배포 세션 시작")
...
def main():
    ...
    try:
        ...
        # ---------------------------------------------------------------------
        # [단계 2] Agent Platform 및 Cloud Run 필수 API 활성화
        # ---------------------------------------------------------------------
        logger.step(2, total_steps, "Agent Platform(aiplatform) 및 Cloud Run 관련 API 활성화")
        required_apis = [
            "run.googleapis.com",
        ...
        # ---------------------------------------------------------------------
        # [단계 3] ADC 서비스 계정에 Agent Platform 권한 부여
        # ---------------------------------------------------------------------
        logger.step(3, total_steps, "Cloud Run 서비스 계정에 Agent Platform(Vertex AI) 접근 권한 부여")
        ...
        logger.success("ADC 서비스 계정 권한 설정 완료 (Secret Manager 불필요)")

        # ---------------------------------------------------------------------
        # [단계 4] Cloud Run 서비스 빌드 및 배포 (키리스 ADC 배포)
        # ---------------------------------------------------------------------
        logger.step(4, total_steps, f"Cloud Run 서비스({SERVICE_NAME}) 빌드 및 키리스(Keyless) 배포")
        logger.log(f"배포 디렉터리: {BASE_DIR}")

        # 키를 환경변수나 시크릿으로 주입하지 않고, 순수 ADC 방식으로 배포합니다.
        deploy_cmd = (
            ...
            f"--set-env-vars=\"GOOGLE_CLOUD_PROJECT={PROJECT},GOOGLE_CLOUD_LOCATION={REGION}\" "
            f"--min-instances=0 "
            f"--max-instances=10 "
            f"--memory=512Mi "
            f"--cpu=1 "
        ...
        run_command(deploy_cmd, f"Cloud Run ({SERVICE_NAME}) ADC 배포 진행", cwd=BASE_DIR)
        logger.success("Cloud Run ADC 컨테이너 빌드 및 배포 완료!")

        # ---------------------------------------------------------------------
        # [단계 5] Cloud Run 공인 HTTPS 서비스 URL 조회
        # ---------------------------------------------------------------------
        logger.step(5, total_steps, "배포된 Cloud Run 서비스 URL 조회")
        url_res = run_command(
        ...
        # ---------------------------------------------------------------------
        # [단계 6] 헬스체크 및 실시간 Gemini 2.5 Flash 대화 테스트 (ADC 인증)
        # ---------------------------------------------------------------------
        logger.step(6, total_steps, "서비스 헬스체크 및 Gemini 2.5 Flash 실시간 대화 API 테스트 (ADC)")
        ...
        # 2) Gemini 2.5 Flash 실시간 채팅 API 테스트
        chat_url = f"{service_url}/api/chat"
        logger.log(f"\n2) Gemini 2.5 Flash 실시간 채팅 API 테스트 (ADC 인증): {chat_url}")
        test_payload = {
            "model": "gemini-2.5-flash",
            "messages": [
                {"role": "user", "content": "안녕하세요! ADC(애플리케이션 기본 사용자 인증 정보) 방식으로 작동 중인 Gemini인가요? 한 줄로 답변해주세요."}
            ],
            "enable_search": False
        }
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=20) as resp:
                ...
                if "done" in raw_stream:
                    logger.success("Gemini 2.5 Flash ADC 실시간 대화 API 테스트 성공!")
                else:
                    logger.log("  스트림 수신 완료")
        except Exception as e:
            logger.log(f"  [대화 API 테스트 알림]: {e}")

        logger.banner("Google Cloud Run 2 (ADC 방식) 챗봇 배포 성공 완료!")
        logger.log(f"🚀 Cloud Run 2 공인 HTTPS 서비스 접속 URL: {service_url}")
